...
def negate_target(neg_file):
    with open(neg_file, 'r') as neg_read, open('wrong-place.csv', 'w') as wrong_file:

        neg_reader = list(csv.reader(neg_read, delimiter=','))[1:]
        wrong_writer = csv.writer(wrong_file, delimiter=',')
        boolList = []
        wrong_writer.writerow(['Target', 'Prediction', 'Source Length', 'Target Verb'])
            ...
            if correct == 1:
                ...
                if 'not' in predsent and targverb in predsent:
                    ...
                    if predsent[predsent.index('not')  + 1] == targverb:
                        ...
                        wrong_writer.writerow([line[0], line[1], sourcelen, targverb])
                else:
                    boolList.append([sourcelen, targlen, predlen, 0])
